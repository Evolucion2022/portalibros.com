<?php
/**
 * Template: My Account (Custom wrapper with premium header + footer)
 *
 * This template is loaded via the template_include filter in functions.php
 * for the WooCommerce My Account page and its sub-endpoints.
 */
...
// Enqueue shop.css + home.css + account.css is already handled in functions.php
...
<main class="site-main" style="background: var(--home-bg, var(--shop-bg, #F6F4F0)); min-height: 60vh;">
    <div style="max-width: 1200px; margin: 0 auto; padding: 40px 24px 80px; box-sizing: border-box;">
        <?php
        while (have_posts()):
            the_post();
            the_content();
